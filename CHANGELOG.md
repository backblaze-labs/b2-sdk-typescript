...
- **Uploads now retry transient failures with fresh upload URLs.** Single-file uploads, multipart parts, stream-backed multipart uploads, and `createWriteStream()` evict a failed upload URL, back off with the shared retry budget, fetch a fresh upload URL / part URL, and retry there for 408/429/5xx, expired upload tokens, network failures, and lost upload response bodies. `onUploadRetry` reports file name, part number, attempt, delay, and classified error before each retry. If the first upload POST succeeded but the response was lost, a retry can create a duplicate file version; compare returned file IDs and SHA-1 values when reconciling uploads, and use lifecycle or version-retention rules when buckets need automatic cleanup.
