...
- **`FileSource(path)` / `FileSource.fromPath(path)` for Node local-file uploads.** `FileSource` is exported from the main entry and `@backblaze-labs/b2-sdk/streams`, supports random-access `slice()` reads for multipart uploads, rejects non-regular files and path swaps on a best-effort filesystem identity check, and lets local-to-B2 sync upload files without first buffering them with `readFile`. It detects size/mtime changes and, on POSIX platforms, ctime changes so same-size rewrites with restored mtime are rejected; use an independent digest when tamper-resistant integrity is required. `fromPath()` performs asynchronous filesystem validation for async hot paths. `toContentSource(input, size)` also accepts async iterables such as Node `Readable` streams for forward-only uploads.
- **Multipart cleanup diagnostics.** Multipart upload, streaming upload, and multipart copy options now accept `onCleanupFailure`, and the root export includes `CleanupFailureEvent` / `CleanupFailureListener`, so failed best-effort `b2_cancel_large_file` cleanup is observable with the orphaned `fileId`.
...
- **`REALM_URLS` is now a frozen informational map.** Runtime realm resolution uses the built-in verified aliases plus direct custom URLs passed to `B2Client`; mutating `REALM_URLS` for custom aliases is unsupported.
...
- **Real-B2 integration workflow** (`.github/workflows/integration.yml`) runs the integration suite sequentially across Node 22.3 + 24 with `max-parallel: 1`, on push, PR, weekly schedule, and `workflow_dispatch`. Defensive `sdk-test-*` bucket sweep at startup absorbs leftovers from crashed runs.
