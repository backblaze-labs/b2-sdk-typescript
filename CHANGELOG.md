...
- **First-party S3-only helper boundary.** `@backblaze-labs/b2-sdk/s3` now exports `createS3CompatibleClient()` for B2-scoped S3-compatible operations: bucket reachability and location checks, ranged/saved GetObject reads, ListObjectsV2, bucket lifecycle updates, multipart create/complete/abort/list-parts/list-uploads/upload-part-copy, and presigned UploadPart URLs. Every network helper accepts an optional `AbortSignal` and has a bounded request / consumed-body timeout by default. `getObject({ saveToPath })` is confined to a configured download root, rejects traversal/symlink/existing destinations, and publishes through a temporary file after the body finishes. `presignUploadPart()` can bind `Content-Length` for untrusted uploaders, and the README documents retry, idempotency, and presigned URL secret-handling requirements. Closes #154.
