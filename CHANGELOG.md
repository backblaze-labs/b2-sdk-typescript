...
- **Multipart resume no longer trusts server part SHA-1 by default.** `resume: true` still reuses the selected unfinished large file, but already-uploaded parts are re-uploaded unless callers set `trustServerPartSha1s: true` for buckets where all writers are mutually trusted.
