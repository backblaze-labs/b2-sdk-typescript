...
- **Multipart resume now verifies unfinished large-file identity before reuse.** Automatic `resume: true` discovery checks upload options, caller file info, encryption, Object Lock settings, and uploaded part lengths before reusing a same-name unfinished file; it does not write SDK-managed resume keys into durable file metadata. SSE-C is excluded from automatic and explicit resume because B2 does not expose customer-key identity, and explicit `resumeFileId` now verifies the target before upload starts. Incompatible unfinished uploads are left for caller cleanup or lifecycle rules.
