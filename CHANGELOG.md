...
- **Realm URLs are validated before authorization sends credentials.** Custom
  ...
  hostnames such as `localhost`, and non-loopback plaintext HTTP now throw
  `B2RealmConfigurationError` before the application key is sent. Accepted
  custom HTTPS hosts are trusted with the application key, so do not derive
  `realm` from untrusted input. Loopback HTTP sends application-key credentials
  unencrypted and is intended only for local simulator or proxy testing.
