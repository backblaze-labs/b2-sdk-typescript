...
- **Sync transfers now stream large files without whole-file buffering.** Local-to-B2 sync uses `FileSource` so multipart uploads read file ranges from disk, and B2-to-local sync streams downloads into deterministic same-directory temp files, verifies the completed byte count, then atomically renames into place only after the stream and checksum checks succeed. Download paths reject traversal and symlink escapes before writing, remove failed temp files, and propagate abort signals to in-flight bucket upload/download calls. Operators syncing active logs or databases should retry after writers quiesce if a file changes during upload; the SDK surfaces that as `FileSource file changed after validation`.
