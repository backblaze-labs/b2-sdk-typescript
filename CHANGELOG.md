...
- **Correct browser SHA-1 for buffer-backed views.** The WebCrypto path of `sha1Hex` / `IncrementalSha1` hashed the entire backing `ArrayBuffer` instead of the `Uint8Array` view's `byteOffset`/`byteLength`, so any subarray (e.g. a carved multipart part) produced a wrong digest in browsers and other non-Node runtimes. Now hashes exactly the view's bytes. The Node `crypto` path was already correct.
