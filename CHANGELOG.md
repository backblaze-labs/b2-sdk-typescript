...
- **Response encryption metadata now uses the public B2 shape.** File-version and unfinished-large-file responses expose SSE-C as mode plus algorithm only, matching B2 responses. Callers that upload with SSE-C must keep the customer key and MD5 separately instead of reading them back from response objects.
