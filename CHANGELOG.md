...
- **`realm: 'staging'` now resolves to the staging authorize endpoint.** It previously
  ...
  split matters for the deployment.
...
- **Redirect following is disabled by default in `FetchTransport`.** Redirect
  responses now surface as a non-retryable `B2RedirectError`, exported from the
  package root and `@backblaze-labs/b2-sdk/errors`, with request and Location
  URLs sanitized before they appear in error messages. Pass
  `followSameHostRedirects: true` to follow guard-checked same-origin GET/HEAD
  redirects; POST redirects and cross-origin redirects remain blocked. Browser
  and edge runtimes may report cross-origin manual redirects as opaque
  redirects with no readable Location, so they are blocked even with the option
  enabled. Monitor or catch `B2RedirectError` during rollout if your deployment
  might depend on CDN, proxy, or regional redirect behavior.
