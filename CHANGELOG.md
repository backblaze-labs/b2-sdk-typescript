...
- **Sync transfers honor abort signals and stalled downloads fail fast.** Upload, download, and B2 copy actions receive `SyncOptions.signal` for in-flight cancellation, and built-in scanners check the signal during enumeration. B2-to-local download body reads now use `downloadIdleTimeoutMs` (default 60 seconds) so a half-open response stream releases its concurrency slot with an error instead of hanging indefinitely.
