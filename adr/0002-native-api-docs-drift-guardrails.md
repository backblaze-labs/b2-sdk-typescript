...
conflict with the SDK's validated wire model:

- `b2_copy_file` is rendered as returning an array, while B2 returns one copied
  file-version object.
- `lifecycleRules` is rendered as `object` in some field tables, while request
  examples and B2 bucket responses use a JSON array.
- `replicationConfiguration` is rendered inconsistently: bucket response examples
  ...
  replication object. Create and update requests use the direct object; bucket
  responses use `{ isClientAuthorizedToRead, value }`.
...
Guard the three tracked shapes in the gated live integration suite:
...
- The same suite asserts that live bucket update and list responses expose
  `lifecycleRules` as arrays.
- The same suite asserts that live bucket responses expose
...
2. Live contract tests are the regression guard for docs-generation drift when
   B2 credentials are available; they skip normally without credentials.
