/**
 * Live multipart contract coverage against a real Backblaze B2 account.
 *
 * Requires env vars:
...
function makeBucketName(): string {
  ...
  if (runId !== undefined && runId !== '') {
    return `sdk-it-${runId}-${runAttempt}-multipart-${Date.now()}`
  }
  return `sdk-it-multipart-${runToken}`
}
...
async function readRequiredBody(body: ReadableStream<Uint8Array> | null): Promise<Uint8Array> {
...
async function emptyBucket(bucket: Bucket): Promise<void> {
  ...
  if (cleanupErrors.length > 0) {
    throw new AggregateError(
      cleanupErrors,
      `empty bucket failed for ${cleanupErrors.length} item(s)`,
    )
  }
}
...
async function createRawMultipartFile(
  ...
  try {
    ...
    expect(listed.nextPartNumber).toBeNull()
    expect(listed.parts.map((part) => part.partNumber)).toEqual([1, 2])
...
describe.skipIf(skip)('B2 multipart integration contract', () => {
  let client: B2Client
  let bucket: Bucket
  ...
  it('uses a download authorization token to fetch scoped bytes', async () => {
    ...
    expectBytesEqual(new Uint8Array(await response.arrayBuffer()), data)
  })
})
