...
> **Requirements for the OIDC path:** npm CLI ≥ 11.5.1 and Node ≥ 22.14.0. The artifact build job pins Node 22.22.2, while the publish-only job uses Node 24 so the GitHub-hosted runner carries npm 11 for trusted publishing. pnpm remains pinned via `packageManager` for install/build/test steps.
