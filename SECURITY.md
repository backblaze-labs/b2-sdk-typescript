...
- **Partner / Computer Backup authorization**: `authorizePartner` sends the Master Application Key only to a verified authorize realm (HTTPS, or an explicitly-allowed loopback host for local testing); untrusted or plaintext realms are rejected before credentials are sent. The Partner token and any returned member/trial application keys are redacted from `toJSON()`, `toString()`, and `util.inspect`, and the same realm-derived SSRF guard locks the transport after authorization.
