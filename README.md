...
When you want S3 compatibility instead — for tooling that already speaks S3, browser direct uploads, or for the Bandwidth Alliance proxy pattern — `@backblaze-labs/b2-sdk/s3` exposes `createS3ClientConfig()`, `createS3CompatibleClient()`, `presignS3GetObjectUrl()`, and `presignS3PutObjectUrl()` so the same SDK covers both surfaces. `createS3CompatibleClient()` is the stable first-party path for the scoped S3-only operations the B2 SDK intentionally supports: `headBucket`, `getBucketLocation`, `getObject` with optional `range` and `saveToPath`, `listObjectsV2`, `putBucketLifecycle`, `presignUploadPart`, and multipart `create`, `complete`, `abort`, `listParts`, `listUploads`, and `uploadPartCopy`. Multipart control operations are exposed through the `multipart` namespace only. Every network helper accepts a `signal` field for request cancellation and has a 15 minute request / consumed-body timeout by default; pass `requestTimeoutMs` on the client or a single call to raise it, or set it to `0` only when caller-owned `AbortSignal` deadlines are enforced.

The first-party S3-compatible client performs exactly one signed HTTP request per helper call and does not retry automatically. Retry `multipart.create` only when duplicate in-progress uploads are acceptable or you reconcile/list/abort duplicates afterward. Retry `multipart.complete` only after checking whether the object may already have been committed, because a lost response can make completion ambiguous. `multipart.abort` is a best-effort cleanup operation and can be retried when the target upload still exists. Retry `multipart.uploadPartCopy` only when replacing the same part number is acceptable. `putBucketLifecycle()` exposes B2-supported lifecycle values only: enabled rules, abort-incomplete-multipart cleanup, expiration, noncurrent-version expiration, and `expiredObjectDeleteMarker: true`.

For Node downloads, `getObject({ saveToPath })` requires `downloadRoot` on the client or the call. The destination must be a relative path below that root; absolute paths, `.`, `..`, symlinked parents, symlink leaves, and existing destinations are rejected. The SDK writes the body to a unique temporary file in the destination directory and publishes it only after the stream completes, cleaning up the temporary file on in-process failures. The default returned-stream path (`getObject()` without `saveToPath`) hands the body stream to the caller, so callers own any longer-lived read deadline after the helper returns.

`presignUploadPart` mints a bearer PUT URL for one multipart part. Do not log the URL or its query string, keep expiry short, and treat it like credential material until it expires. When a URL is handed to an untrusted uploader, pass `contentLength` so the generated URL signs `Content-Length` and the uploader cannot use the same bearer URL for a larger body. You may also sign an endpoint-supported checksum header with `checksumSha256` when integrity needs to be bound at upload time.
