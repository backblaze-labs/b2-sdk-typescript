...
`SyncFolder.scan()` implementations must yield paths sorted with `compareSyncRelativePaths`, exported from `@backblaze-labs/b2-sdk/sync`, because `zipFolders()` merge-joins using that ordering. Custom scanners that set `appliesScanFilters: true` can reuse the exported `pathPassesSyncFilters`, `directoryMayContainSyncPaths`, `filterSyncPaths`, `literalPrefixForSyncFilters`, and `pathSkippedByRegExpInputLimit` helpers. The built-in local and B2 scanners currently sort before yielding, so large trees or prefixes may require memory proportional to the scanned entries. B2 scans also group listed versions before yielding, and `synchronize()` buffers generated actions before executing them; exclude filters and non-literal includes do not bound that listing or action footprint. Built-in B2 skip diagnostics use the `SyncSkipReason` values `outside-prefix`, `unsafe-name`, `relative-path-collision`, and `path-too-long-for-regexp`; `synchronize()` may aggregate excess scanner diagnostics with `scan-skip-overflow`.

B2-to-local downloads stream into a unique temp file in the destination directory and rename it into place so a swapped leaf symlink is replaced instead of followed. Downloads and local deletes reject symlinked path components under the local root and revalidate immediately before the final filesystem operation. The local sync root should not be writable by concurrent untrusted processes; Node does not expose a portable way for the SDK to prevent a parent directory inside the root from being replaced between the final validation and rename or unlink.
