...
Object Lock state must also be verified before reuse. If the caller requests `fileRetention` or `legalHold`, unreadable candidate fields are treated as incompatible because the SDK cannot verify the requested state. If those options are omitted, unreadable retention or legal-hold fields are accepted for resume.
