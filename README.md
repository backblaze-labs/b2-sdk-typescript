...
Object Lock state must also be verified before reuse. Unreadable retention or legal-hold fields are incompatible for both `resume: true` and `resumeFileId` because the SDK cannot prove the unfinished file is unlocked or matches the requested lock state.
