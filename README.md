...
Glob strings use the SDK dialect: `*` and `?` stay within one path segment, a whole `**` segment crosses directories, slash-less patterns match any basename or ancestor directory, and excludes win over includes. RegExp filters are also supported, but accepted patterns are guarded by a best-effort synchronous safety heuristic whose exact subset may change as protections tighten; paths that exceed the RegExp input guard are reported with `reason: 'path-too-long-for-regexp'` when `onSkip` is configured. B2 prefixes are raw object-name prefixes; backslashes are not rewritten to `/`. Include-prefix pushdown for B2 listings stops before normalized path separators so raw backslash-named objects are still listed and filtered client-side.

`SyncFolder.scan()` direct consumers should expect paths sorted with `compareSyncRelativePaths`, exported from `@backblaze-labs/b2-sdk/sync`. During `synchronize()`, custom folders that do not set `appliesScanFilters: true` are filtered and sorted before the merge join for compatibility with older custom scanners. Custom scanners that do set `appliesScanFilters: true` are trusted to enforce the SDK filter dialect and comparator order themselves; reuse the exported `pathPassesSyncFilters`, `directoryMayContainSyncPaths`, `filterSyncPaths`, `literalPrefixForSyncFilters`, and `pathSkippedByRegExpInputLimit` helpers. The built-in local and B2 scanners currently sort before yielding, so large trees or prefixes may require memory proportional to scanned entries. B2 scans also group listed versions before yielding, and exclude filters or non-literal includes do not bound that listing footprint. `SyncOptions.maxScanEntries` / `SyncScanOptions.maxScanEntries` default to the SDK scan ceiling and fail with a defined scan-limit error when exceeded; pass `Infinity` only when the process heap is sized for the full result set. Action outcomes from `synchronize()` are streamed as each action settles and are not retained for the full run. Built-in B2 skip diagnostics use the `SyncSkipReason` values `outside-prefix`, `unsafe-name`, `relative-path-collision`, and `path-too-long-for-regexp`; `synchronize()` may aggregate excess scanner diagnostics with `scan-skip-overflow`.

B2-to-local downloads stream into a unique temp file in the destination directory and rename it into place so a swapped leaf symlink is replaced instead of followed. Stale `.b2sdk-*.partial` files from previous interrupted downloads are removed by local scans and before new downloads in the same directory. Download body reads have a 60 second idle timeout by default; set `downloadIdleTimeoutMs` to tune it or `Infinity` to disable the watchdog. Downloads and local deletes reject symlinked local roots and symlinked path components under the root, then revalidate immediately before the final filesystem operation. The local sync root should not be writable by concurrent untrusted processes; Node does not expose a portable way for the SDK to prevent a parent directory inside the root from being replaced between the final validation and rename or unlink. Per-action failures are emitted as `error` events and the terminal aggregate error includes `failureCount` and `failedPaths` so consumers do not need to parse the message.
