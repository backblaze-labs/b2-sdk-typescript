...
B2-to-local sync skips object names that are unsafe on local filesystems, including NTFS alternate data stream names, DOS device basenames, trailing dot/space names, and case/Unicode-canonical collisions. Downloads stream into an owned unique temp file in the destination directory and rename it into place; local scans are read-only and do not delete `.b2sdk-*.partial`-looking files. Download body reads have a 60 second idle timeout by default; set `downloadIdleTimeoutMs` to tune it or `Infinity` to disable the watchdog. Downloads and local deletes reject symlinked local roots and symlinked path components under the root, then revalidate immediately before the final filesystem operation. Per-action failures are emitted as `error` events; the terminal aggregate error includes `failureCount`, up to 100 `failedPaths`, and `failedPathOmittedCount` when more paths were omitted.
