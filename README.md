...
Object Lock state must also be verified before reuse. Automatic `resume: true` treats unreadable retention or legal-hold fields as incompatible because the SDK cannot prove the unfinished file is unlocked. An explicit `resumeFileId` is the opt-in path for a caller that already trusts that specific unfinished file; unreadable Object Lock fields are still rejected when the caller requested `fileRetention` or `legalHold`.
